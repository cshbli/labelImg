...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        #LL Create a widget for setting default box width and height
        self.useDefautBoxCheckbox = QCheckBox(u'Use default box width, height')
        self.useDefautBoxCheckbox.setChecked(True)
        self.defaultBoxWidth = QLineEdit()
        self.defaultBoxWidth.setFixedWidth(50)
        self.defaultBoxWidth.setText(str(20))
        self.defaultBoxHeight = QLineEdit()
        self.defaultBoxHeight.setFixedWidth(50)
        self.defaultBoxHeight.setText(str(20))
        #LL
        ...
        #LL
        useDefautBoxQHBoxLayout = QHBoxLayout()       
        useDefautBoxQHBoxLayout.addWidget(self.useDefautBoxCheckbox)
        useDefautBoxQHBoxLayout.addWidget(self.defaultBoxWidth)
        useDefautBoxQHBoxLayout.addWidget(self.defaultBoxHeight)
        #LL
        ...
        #LL
        useDefautBoxContainer = QWidget()
        useDefautBoxContainer.setLayout(useDefautBoxQHBoxLayout)
        #LL
        ...
        #LL
        listLayout.addWidget(useDefautBoxContainer)
        #LL
        ...
        fitWidth = action('Fit &Width', self.setFitWidth,
                          ...
                          checkable=True, enabled=False)
        incBright = action('Increase &Bright', partial(self.setBright, 20),
                            'Ctrl+B', 'increase brightness', u'Increase brightness', enabled=False)
        # Group zoom controls into a list for easier toggling.
        zoomActions = (self.zoomWidget, zoomIn, zoomOut,
                       zoomOrg, fitWindow, fitWidth, incBright)
        ...
        self.actions.beginner = (
            ...
            zoomIn, zoom, zoomOut, incBright, fitWindow, fitWidth)
    ...
    def setBright(self, increment=20):
        scale = 1 + increment / 100
        img = convertQImageToMat(self.image, QImage.Format_RGB888)
        bimg = np.where(img * scale < 255, img * scale, 255).astype(img.dtype)
        
        height, width, channel = bimg.shape
        bytesPerLine = channel * width
        self.image = QImage(bimg.data, width, height, bytesPerLine, QImage.Format_RGB888)      
        
        self.canvas.loadPixmap(QPixmap.fromImage(self.image))  
              
        # reload label xml file and show bound box according to its filename
        if self.usingPascalVocFormat is True:
            if self.defaultSaveDir is not None:
                ...
                xmlPath = self.filePath.split(".")[0] + XML_EXT
    ...
    def loadFile(self, filePath=None):
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            if self.labelList.count():
                ...
                # LL
                # the QListItem doesn't have setItemSelected() method anymore, removing it doesn't seem
                # to affect any features; maybe setCurrentItem() covers the lost function.
...
def convertQImageToMat(qimg, format):
    ...
    arr = np.array(ptr)
    arr = arr.reshape(height, width, arr.shape[0] // (height * width))  #  Copies the data
    return arr
